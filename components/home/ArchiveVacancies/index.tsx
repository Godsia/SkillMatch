...
export default function ArchiveVacanciesPage() {
    ...
    const loadVacancies = async () => {
        setIsLoading(true);
        try {
            const data = await vacanciesApi.getDisliked();
            setVacancies(data);
        } catch (error: any) {
            console.error('Ошибка загрузки архива:', error);
            Alert.alert(
                'Ошибка',
                error.response?.data?.message || 'Не удалось загрузить архив вакансий.'
            );
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={listStyles.header}>
                <Text style={listStyles.headerTitle}>Архив</Text>
            </View>
            {isLoading ? (
                ...
                <View style={listStyles.centered}>
                    <Text style={listStyles.emptyText}>Архив пуст</Text>
                    <Text style={listStyles.emptySubtext}>Отклонённые вакансии появятся здесь</Text>
                </View>
            ) : (
                <FlatList
                    data={vacancies}
            ...
            <View style={styles.bottomNav}>
                <Pressable style={styles.navItem} onPress={() => navigation.navigate('Home' as never)}>
                    <View style={styles.navIcon}>
                        <Text style={styles.heartIcon}>🀆</Text>
                    </View>
                </Pressable>
                <Pressable style={styles.navItem} onPress={() => navigation.navigate('SelectedVacancies' as never)}>
                    <View style={styles.navIcon}>
                        <Text style={styles.heartIcon}>♡</Text>
